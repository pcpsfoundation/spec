...
- Restructured document: family is now the root object
- Added `guardians` array with roles
- Added `children` array wrapping individual policies
- Added `devices` array per child
- Added `family_id`, `family_name` top-level fields
- Moved `child_id`, `child_name`, `child_age` into child entries
- Simplified `content.age_ratings` to a single `content.max_age_rating` integer — platform adapters map to PEGI, ESRB, etc.
- Removed `app_id` from app entries — apps are identified by name, adapters resolve to platform identifiers
- Policy sections otherwise unchanged from 1.0

### 1.0 (2026-02-06)

- Initial release (per-child document)
